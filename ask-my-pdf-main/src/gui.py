...
def ui_info():
	...
	Question answering system built on Financial Doocuments.
	""")
	ui_spacer(1)
	st.write("Made by BlinkX .", unsafe_allow_html=True)
	ui_spacer(1)
	st.markdown("""
	
		""")
	ui_spacer(1)
	st.markdown('Please raise your cocerns  [here](https://blinkx.in/).')
...
def b_ask():
	...
	#
	disabled = 0
	# (not ss.get('api_key') and not ss.get('community_pct',0)) or not ss.get('index')
